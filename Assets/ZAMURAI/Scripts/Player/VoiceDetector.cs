...
public class VoiceDetector : MonoBehaviour
{
	...
	public string ModelPath = "vosk-model-small-ja-0.22.zip";
    ...
    [SerializeField] VoiceCommand voiceCommand;
	...
	public Action<PointActionType> OnTranscriptionResult;
    ...
    private void MakeCommandList()
    {
        ...
        foreach (var command in voiceCommand.PointCommands)
        {
            keywords.Add(new JSONString(command.commandName.ToLower()));
        }
	...
	public void SwitchRecording(bool isRecording = false)
	{
        if (isRecording == _running) return; // すでにその状態なら何もしない
	...
	void Update()
	{
		...
		{
            Debug.Log($"Transcribed command: {voiceResult}");
            PointActionType command = (PointActionType)Enum.Parse(typeof(PointActionType), voiceResult);
		    OnTranscriptionResult?.Invoke(command);
		}
	}
	...
	private async Task ThreadedWork()
	{
		...
		if (!_recognizerReady)
		{
			...
			{
                Debug.LogWarning("No keywords specified for Vosk. Recognizer will detect any phrase in the model's dictionary.");
				return;
			}
			else
			{
				Debug.Log(_grammar);
				_recognizer = new VoskRecognizer(_model, 16000, _grammar);
			}
    ...
    public void EnqueueExternalAudio(short[] samples)
    {
        if (!_running) return; // 認識中でなければ無視

        // Photonスレッドから届いたデータをキューに積む
        if (!_running) return;

		// 参照渡しではなく、中身を複製して渡す（これが必要！）
		short[] copy = new short[samples.Length];
		System.Array.Copy(samples, 0, copy, 0, samples.Length);
		_threadedBufferQueue.Enqueue(copy);
    }
